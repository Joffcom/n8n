			...
			try {
				if (resource === 'bill') {
					...
					if (operation === 'create') {
							...
							const body = {
								Id: this.getNodeParameter('vendorcreditId', i),
								SyncToken: await getSyncToken.call(this, i, companyId, resource),
							} as IDataObject;

							const endpoint = `/v3/company/${companyId}/${resource}`;
							responseData = await quickBooksApiRequest.call(this, 'POST', endpoint, qs, body);
							responseData = responseData['VendorCredit'];
							...
							const creditId = this.getNodeParameter('creditId', i);
							const endpoint = `/v3/company/${companyId}/${resource}/${creditId}`;
							responseData = await quickBooksApiRequest.call(this, 'GET', endpoint, {}, {});
							responseData = responseData['VendorCredit'];
